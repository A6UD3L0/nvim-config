...
-- Utilities
-- <leader>u namespace for utilities and tools
map("n", "<leader>u", "<cmd>UndotreeToggle<CR>", { desc = "Toggle Undotree" })
map("n", "<leader>ht", function()
  -- This mapping is defined in the hardtime plugin itself, but adding here for discoverability
  -- Call the toggle directly since we configured it in the plugin setup
  vim.cmd("lua require('hardtime').toggle()")
  local state = require("hardtime").is_enabled() and "enabled" or "disabled"
  vim.notify("Hardtime " .. state, vim.log.levels.INFO)
end, { desc = "Toggle Hardtime (break bad habits)" })
