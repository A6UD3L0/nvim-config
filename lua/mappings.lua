...
-- Unified keymapping and which-key configuration for Neovim
-- Combines ThePrimeagen's mappings, NvChad simplicity, and advanced which-key UX
...
-- Group colors for which-key (optional, for highlights)
local group_colors = {
  Find        = "#9ECE6A",
  Git         = "#F7768E",
  LSP         = "#2AC3DE",
...
-- Descriptive icons for groups (optional)
local key_icons = {
  f = "", g = "", l = "", b = "", t = "", w = "", e = "", x = "", a = "", h = "", z = "", u = "", k = "", p = "", r = "", c = "", s = "", q = "",
}

-- Main which-key setup and registration
function M.setup()
  ...
  -- Optionally set group highlights
  ...
  which_key.setup({
    plugins = {
      marks = true, registers = true, spelling = { enabled = true, suggestions = 20 },
      presets = {
        operators = true, motions = true, text_objects = true, windows = true, nav = true, z = true, g = true,
      },
    },
    icons = {
    ...
    window = {
      border = "rounded", position = "bottom", margin = { 1, 0, 1, 0 }, padding = { 1, 2, 1, 2 }, winblend = 0,
    },
    layout = {
      height = { min = 4, max = 25 }, width = { min = 20, max = 50 }, spacing = 3, align = "left",
    },
    ignore_missing = true,
    show_help = true,
    triggers = "auto",
  ...
  -- Register which-key groups (example, extend as needed)
  which_key.register({
    ["<leader>w"] = { name = key_icons.w .. "+windows" },
    ["<leader>f"] = { name = key_icons.f .. "+find" },
    ["<leader>g"] = { name = key_icons.g .. "+git" },
    ["<leader>t"] = { name = key_icons.t .. "+terminal" },
    ["<leader>p"] = { name = key_icons.p .. "+project" },
    ["<leader>u"] = { key_icons.u .. "Toggle UndoTree" },
    ["<leader>e"] = { key_icons.e .. "Toggle file explorer" },
    ["<leader>pu"] = { key_icons.p .. "UV: Install Python deps" },
    ["<leader>pv"] = { key_icons.p .. "UV: Create venv" },
    ["<leader>pa"] = { key_icons.p .. "UV: Add package under cursor" },
    ["<leader>pr"] = { key_icons.p .. "UV: Remove package under cursor" },
    ["<leader>ps"] = { key_icons.p .. "UV: Sync environment" },
    ["<leader>pt"] = { key_icons.p .. "UV: List packages" },
    ["<leader>pl"] = { key_icons.p .. "UV: List venvs" },
    ["<leader>pd"] = { key_icons.p .. "UV: Destroy current venv" },
  })
end

-- Example keymaps (add/extend as needed)
local map = vim.keymap.set
...
-- LSP/diagnostic setup functions
